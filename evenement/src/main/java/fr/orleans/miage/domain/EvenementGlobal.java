...
public class EvenementGlobal {
    ...
    @JsonIgnoreProperties("evenementGlobal")
    @Temporal(TemporalType.TIMESTAMP)
    @OneToMany(mappedBy = "evenementGlobal", fetch = FetchType.EAGER, cascade = CascadeType.ALL)
    ...
    public void addHoraires() throws ParseException {
        ...
        for(String horaire : horaires){
            ...
            this.evenements.add(new Evenement(format.parse(horaire_[0]), format.parse(horaire_[1]), this));
        }
    }
    ...
    public void setEvenements(Collection<Evenement> evenements) { this.evenements = evenements; }
